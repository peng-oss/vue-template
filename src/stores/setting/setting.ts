...
export const useSettingStore = defineStore('setting', () => {
  ...
  const defaultOption = ref({
    ...
    limitScrollNum: 3, // 开始无缝滚动的数据量 this.dataList.length
    singleHeight: 0, // 单步运动停止的高度(默认值0是无缝不停止的滚动) direction => 0/1
    singleWidth: 0, // 单步运动停止的宽度(默认值0是无缝不停止的滚动) direction => 2/3
    singleWaitTime: 6*1000, // 单步运动停止的时间(默认值1000ms)
  ...
  const initSetting = () => {
    ...
    let useName: string | null = localStorage.getItem('username')
  ...
  return { settingShow, setSettingShow,userName, isScale, setUserName , setIsScale, initSetting, setSettingData, defaultOption, indexConfig, setIndexConfig }
})
